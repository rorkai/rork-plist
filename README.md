...
Decoding uses the fastest codec the runtime has — `Uint8Array.fromBase64`, then `Buffer`, then plain JavaScript — and behaves identically on all of them, down to the error messages.
...
‡ [`@plist/plist`](https://www.npmjs.com/package/@plist/plist) also offers OpenStep serialization, but not losslessly: probing it shows `Uint8Array` data serialized as an index-keyed dictionary (`{0 = 1; 1 = 2;}`) rather than a `<hex>` literal, so payloads do not survive a round trip, and its OpenStep parser reads `<0102>` data literals back as empty objects. It is therefore not comparable on this operation.
