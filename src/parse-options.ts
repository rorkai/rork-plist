...
export interface ParsePlistOptions {
   ...
   *
   * - `"copy"` (the default) copies each payload out, so parsed values own
   ...
   * - `"view"` returns each payload as a `subarray` view aliasing the input
   *   buffer, skipping the copy. Use it to pull a few fields out of a large
